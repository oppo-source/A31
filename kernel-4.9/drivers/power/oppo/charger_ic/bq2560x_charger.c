...
int bq2560x_reset_chip(struct bq2560x *bq)
{
	int ret;
#ifdef ODM_HQ_EDIT
/*zhouyi@ODM.HQ.BSP.CHG.Basic 2019.12.18 add bq reset function*/
	ret = bq2560x_update_bits(bq, BQ2560X_REG_00, 0xFF, 0x0);
	if (ret < 0) {
		pr_info("Couldn't reset REG00 rc = %d\n", ret);
	}
	ret = bq2560x_update_bits(bq, BQ2560X_REG_01, 0xFF, 0x1A);
	if (ret < 0) {
		pr_info("Couldn't reset REG01 rc = %d\n", ret);
	}
	ret = bq2560x_update_bits(bq, BQ2560X_REG_02, 0xFF, 0xA2);
	if (ret < 0) {
		pr_info("Couldn't reset REG02 rc = %d\n", ret);
	}
	ret = bq2560x_update_bits(bq, BQ2560X_REG_03, 0xFF, 0x22);
	if (ret < 0) {
		pr_info("Couldn't reset REG03 rc = %d\n", ret);
	}
	ret = bq2560x_update_bits(bq, BQ2560X_REG_04, 0xFF, 0x88);
	if (ret < 0) {
		pr_info("Couldn't reset REG04 rc = %d\n", ret);
	}
	ret = bq2560x_update_bits(bq, BQ2560X_REG_05, 0xFF, 0x1F);
	if (ret < 0) {
		pr_info("Couldn't reset REG05 rc = %d\n", ret);
	}
	ret = bq2560x_update_bits(bq, BQ2560X_REG_07, 0xFF, 0x4C);
	if (ret < 0) {
		pr_info("Couldn't reset REG07 rc = %d\n", ret);
	}
	ret = bq2560x_update_bits(bq, BQ2560X_REG_0A, 0x03, 0x0);
	if (ret < 0) {
		pr_info("Couldn't reset REG0A rc = %d\n", ret);
	}
#else
...
int bq2560x_reset_charger(struct charger_device *chg_dev)
{
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
#ifdef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.12.18 add bq check function*/
	if (atomic_read(&bq->charger_suspended) == 1) {
		pr_info("charger in suspended.\n");
		return 0;
	}
#endif /*ODM_HQ_EDIT*/

	return bq2560x_reset_chip(bq);
}
#endif
